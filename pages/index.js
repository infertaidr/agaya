...
const SYSTEM = {
  en: "You are Agaya, a compassionate fertility health AI assistant. Provide accurate, evidence-based information about reproductive health, fertility, IVF, ovulation, PCOS, and related topics. Always note responses are for educational purposes. Be warm and supportive. Respond in the same language the user writes in.",
  ko: "당신은 Agaya, 불임 건강 AI 어시스턴트입니다. 생식 건강, 불임, IVF, 배란, PCOS에 대한 정확한 정보를 제공하세요. 항상 교육 목적임을 안내하고 의료 전문가 상담을 권장하세요. 따뜻하고 공감적으로 답변하세요.",
  sw: "Wewe ni Agaya, msaidizi wa AI wa afya ya uzazi. Toa taarifa sahihi kuhusu afya ya uzazi. Jibu kwa Kiswahili."
};
...
export default function Home() {
  ...
  function calcOvulation() {
    ...
    const ovul = new Date(lmp); ovul.setDate(lmp.getDate() + cycleLen - 14);
    const next = new Date(lmp); next.setDate(lmp.getDate() + cycleLen);
    const fStart = new Date(ovul); fStart.setDate(ovul.getDate() - 5);
    const fEnd = new Date(ovul); fEnd.setDate(ovul.getDate() + 1);
    const fmt = d => d.toLocaleDateString('ko-KR', { month: 'long', day: 'numeric' });
    setOvulResult({ next: fmt(next), ovul: fmt(ovul), fertile: `${fmt(fStart)} – ${fmt(fEnd)}` });
  }
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid #e5e7eb', marginBottom: 24 }}>
        {[['chat','💬 채팅'],['ovulation','📅 배란'],['symptoms','📋 증상'],['image','🔬 이미지']].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} style={{ padding: '10px 16px', fontSize: 14, cursor: 'pointer', color: tab===id ? '#1D9E75' : '#6b7280', borderBottom: tab===id ? '2px solid #1D9E75' : '2px solid transparent', background: 'transparent', border: 'none', borderBottom: tab===id ? '2px solid #1D9E75' : '2px solid transparent', fontWeight: tab===id ? 600 : 400 }}>
            {label}
          </button>
        ))}
      </div>
          ...
          <div style={{ minHeight: 280, maxHeight: 320, overflowY: 'auto', background: '#f9fafb', borderRadius: 12, padding: 16, marginBottom: 12, border: '1px solid #e5e7eb' }}>
            ...
            {loading && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                ...
                <div style={{ background: 'white', padding: '8px 12px', borderRadius: 12, border: '1px solid #e5e7eb' }}>⏳</div>
              </div>
            )}
          </div>
      ...
      {tab === 'ovulation' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div>
              <label style={{ display: 'block', fontSize: 13, color: '#6b7280', marginBottom: 6 }}>마지막 생리 시작일</label>
              <input type="date" value={lmpDate} onChange={e => setLmpDate(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #d1d5db', fontSize: 14 }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 13, color: '#6b7280', marginBottom: 6 }}>생리 주기 (일)</label>
              <input type="number" value={cycleLen} onChange={e => setCycleLen(Number(e.target.value))} min={21} max={45} style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #d1d5db', fontSize: 14 }} />
            </div>
          </div>
          <button onClick={calcOvulation} style={{ width: '100%', padding: 10, background: '#1D9E75', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 14, marginBottom: 16 }}>계산하기</button>
          {ovulResult && (
            <div style={{ background: '#E1F5EE', borderRadius: 12, padding: '1.25rem', border: '1px solid #5DCAA5' }}>
              {[['다음 생리 예정일', ovulResult.next],['배란 예정일', ovulResult.ovul],['가임 기간', ovulResult.fertile]].map(([label, value]) => (
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #9FE1CB' }}>
