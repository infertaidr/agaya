...
export default function Home() {
  ...
  // [수정 5] periods를 localStorage에만 저장 (서버 전송 없음)
  useEffect(() => {
    try {
      if (periods.some(p => p !== '')) {
  ...
  const L = {
    ko: {
      ...
      periodSub: '최근 6개월간 생리 시작일을 입력하면 주기 패턴을 확인할 수 있어요',
      ...
      dataNote: '개 주기 데이터 기반',
      ageLabel: '나이 (세, 선택)',
      amhInfoTitle: 'AMH란?',
    },
    en: {
      periodTitle: '6-Month Period Tracker',
      periodSub: 'Enter your last 6 period start dates to view your cycle pattern',
      ...
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 20 }}>
        <div style={{ background: 'white', borderRadius: 16, padding: 28, maxWidth: 480, width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
            ...
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>Agaya 서비스 안내</h2>
              <p style={{ fontSize: 12, color: '#6b7280', margin: 0 }}>이용 전 반드시 확인해주세요</p>
            </div>
          </div>
          <div style={{ background: '#FEF3C7', borderRadius: 10, padding: '14px 16px', marginBottom: 16, border: '1px solid #FCD34D', fontSize: 13, color: '#92400E', lineHeight: 1.7 }}>
            ...
            본 서비스는 <strong>의료법상 의료행위가 아니며</strong>, 식품의약품안전처 허가 의료기기가 아닙니다.<br/>
            제공되는 모든 정보는 <strong>교육 목적의 일반 정보</strong>이며, 의학적 진단·처방·치료를 대체하지 않습니다.
          </div>
          <div style={{ fontSize: 13, color: '#374151', lineHeight: 1.8, marginBottom: 20 }}>
            <p style={{ margin: '0 0 8px', fontWeight: 600 }}>개인정보 처리방침</p>
            <p style={{ margin: '0 0 6px' }}>• 입력하신 생리 주기 데이터는 <strong>기기 내 브라우저(localStorage)에만 저장</strong>되며, 서버로 전송되거나 외부에 공유되지 않습니다.</p>
            <p style={{ margin: '0 0 6px' }}>• AI 채팅 내용은 응답 생성 후 별도로 저장되지 않습니다.</p>
            <p style={{ margin: '0 0 6px' }}>• 난임 관련 정보는 개인정보보호법상 민감정보로, 최소한의 정보만 처리합니다.</p>
            <p style={{ margin: '0 0 16px' }}>• 응급상황 시 즉시 <strong>119</strong>에 연락하거나 가까운 응급실을 방문하세요.</p>
            <p style={{ margin: 0, fontWeight: 600 }}>이용약관</p>
            <p style={{ margin: '4px 0 0', color: '#6b7280' }}>서비스 이용으로 인해 발생하는 결과에 대해 Agaya 및 서비스 제공자는 의료적 책임을 지지 않습니다. 건강 관련 결정은 반드시 자격을 갖춘 의료 전문가와 상담 후 내리세요.</p>
          </div>
          <button
          ...
          <p style={{ fontSize: 11, color: '#9ca3af', textAlign: 'center', marginTop: 10 }}>동의 후 서비스를 이용하실 수 있어요</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      {tab === 'ovulation' && (
        <div>
          ...
          <div style={{ padding: '10px 14px', background: '#f9fafb', borderRadius: 10, border: '1px solid #e5e7eb', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <input type="number" value={age} onChange={e => setAge(e.target.value)} placeholder="예: 35" style={{ ...iStyle, width: 120 }} />
          </div>
          ...
          {ovulResult && !ovulResult.error && (
            <div>
              <div style={{ background: '#E1F5EE', borderRadius: 12, padding: '1.25rem', border: '1px solid #5DCAA5', marginBottom: 16 }}>
                ...
                {[[L.avgCycle,`${ovulResult.avg}${L.days} (${ovulResult.min}~${ovulResult.max}일)`],[L.nextOvul,ovulResult.ovulDay],['배란 가능 범위 (참고)',ovulResult.ovulRange],[L.fertile,ovulResult.fertile],[L.nextPeriod,ovulResult.nextPeriod]].map(([label,value]) => (
                ...
                <p style={{ fontSize: 12, color: '#6b7280', marginTop: 12 }}>* {ovulResult.cycleCount}{L.dataNote} | 기기 내 저장, 서버 전송 없음</p>
                ...
                <div style={{ marginTop: 12, padding: '10px 12px', background: '#FEF3C7', borderRadius: 8, border: '1px solid #FCD34D', fontSize: 12, color: '#92400E', lineHeight: 1.6 }}>
                  ⚕️ 이 결과는 통계 기반 참고값이며 개인차가 있습니다. 정확한 배란일 확인은 산부인과 초음파 검사를 통해 확인하세요.
                </div>
              </div>

              {/* [수정 1] AMH 수치 해석 제거 → AMH 일반 정보로 대체 */}
              <div style={{ background: '#EFF6FF', borderRadius: 12, padding: '1.25rem', border: '1px solid #BFDBFE' }}>
                ...
                <p style={{ fontSize: 12, color: '#6b7280', marginBottom: 14 }}>AMH 수치 해석은 전문의와 상담하세요. 아래는 교육용 일반 정보예요.</p>
                {AMH_INFO[lang].map((item, i) => (
                ...
                <div style={{ padding: '8px 12px', background: '#FEF3C7', borderRadius: 8, fontSize: 12, color: '#92400E', border: '1px solid #FCD34D' }}>
                  ⚕️ AMH 수치 해석 및 치료 결정은 반드시 불임 전문의와 상담하세요. 본 정보는 교육 목적이며 의료 행위가 아닙니다.
                </div>
              </div>
            </div>
          )}
      ...
      {tab === 'symptoms' && (
        <div>
          <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 12 }}>
            증상을 선택하면 관련 생식 건강 정보와 <strong>전문의 상담 시 논의할 포인트</strong>를 안내해드려요.
          </p>
          ...
          <button onClick={analyzeSymptoms} disabled={symptomLoading} style={{ width: '100%', padding: 10, background: '#1D9E75', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 14 }}>
            {symptomLoading ? '정보 불러오는 중...' : '관련 정보 및 상담 가이드 보기 ↗'}
          </button>
          {symptomResult && (
      ...
      {tab === 'image' && (
        <div>
          <div style={{ padding: '12px 16px', background: '#FEF3C7', borderRadius: 8, border: '1px solid #FCD34D', fontSize: 13, color: '#92400E', marginBottom: 16, lineHeight: 1.7 }}>
            ...
            AI가 제공하는 내용은 교육 목적의 일반 정보이며, <strong>전문의가 판독한 결과가 아닙니다.</strong><br/>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {imgType !== '🧪 배란테스트 참고' && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <input type="text" value={imgQuestion} onChange={e => setImgQuestion(e.target.value)} placeholder="예: 이 이미지에 대해 궁금한 점을 입력하세요" style={iStyle} />
                </div>
              )}
              <button onClick={async () => {
                ...
                try {
                  ...
                  const question = imgType === '🧪 배란테스트 참고'
                    ? '이 배란테스트 스틱의 컨트롤선(C)과 테스트선(T) 색상을 비교해주세요. 색상 진하기를 10점 만점으로 수치화하되, 이것은 교육 목적의 색상 비교 참고 정보이며 전문의 판독 결과가 아님을 명시해주세요.'
                    : (imgQuestion || '이 이미지에서 보이는 것을 교육적으로 설명해주세요. 이것은 진단이 아닌 일반 정보 제공이며 전문의 판독 결과가 아님을 명시해주세요.');
                  const system = imgType === '🧪 배란테스트 참고'
                    ? '당신은 교육용 색상 비교 안내 어시스턴트예요. 배란테스트 스틱의 컨트롤선(C)과 테스트선(T) 색상을 비교하고 10점 만점으로 수치화해주세요. 형식: 색상 비교 점수: X.X/10, 참고: 진함/비슷함/연함, 설명: (간단한 설명). 반드시 "이것은 교육 목적의 색상 비교 참고 정보이며, 전문의가 판독한 결과가 아닙니다. 정확한 판단은 제품 설명서 확인 및 전문의 상담이 필요합니다"라는 문구를 결과 맨 마지막에 추가하세요.'
                    : '당신은 Agaya, 산부인과 전문의가 만든 교육 정보 어시스턴트예요. 이미지를 보고 교육적 일반 정보만 제공해요. 반드시 "이것은 의료법상 의료행위가 아니며, 전문의가 판독한 결과가 아닙니다. 정확한 진단은 반드시 산부인과 전문의와 상담하세요"라는 문구를 결과 마지막에 추가하세요.';
                  ...
                  const data = await res.json();
                  setImgResult(data.content || '결과를 가져올 수 없어요.');
                ...
                {imgLoading ? '🔍 불러오는 중...' : imgType === '🧪 배란테스트 참고' ? '🧪 색상 비교 참고 정보 보기' : '🔬 이미지 일반 정보 보기'}
              </button>
            </div>
          )}
          {/* [수정 3] 결과 바로 옆에 "전문의 판독 결과 아님" 재강조 */}
          {imgResult && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ padding: '8px 14px', background: '#FEF3C7', borderRadius: '8px 8px 0 0', border: '1px solid #FCD34D', borderBottom: 'none', fontSize: 12, color: '#92400E', fontWeight: 600 }}>
                ⚕️ 전문의가 판독한 결과가 아닙니다 — 교육 목적 참고 정보
              </div>
      ...
      <div style={{ marginTop: 48, padding: '16px 20px', background: '#f9fafb', borderRadius: 12, border: '1px solid #e5e7eb', fontSize: 12, color: '#6b7280', lineHeight: 1.9 }}>
        ...
        <p style={{ margin: '0 0 4px' }}>• 생리 주기 데이터는 기기 내 브라우저에만 저장되며 서버로 전송되지 않습니다.</p>
