...
function getAMHInterpretation(amh, age) {
  ...
  else if (amh >= 0.7) { amhLevel = '낮음'; amhMsg = '난소 예비력이 낮아요.'; ovarianAge = `실제 나이(${age}세)보다 난소 나이가 상당히 높아요. 몇 번 자연 시도 후 안 된다면 빠른 전문의 상담이 필요해요.`; }
...
const EMPTY_PROFILE = {
  ...
  otherConditions: '', tryingMonths: '', notes: ''
};
...
export default function Home() {
  ...
  const labels = {
    ...
    en: {
      ...
      tip: '💡 Good to know', tipEgg: '🥚 An egg survives 24-48 hours after ovulation. Ovulation day is typically just one day.',
  ...
  async function sendMessage(text) {
    ...
    const savedProfile = localStorage.getItem('agaya_profile');
    let systemWithProfile = SYSTEM[lang];
    if (savedProfile) {
      try {
        const p = JSON.parse(savedProfile);
        const profileContext = `사용자 정보: 나이 ${p.age||'미입력'}세, AMH ${p.amh||'미입력'} ng/mL, 수술경력: ${p.surgeryHistory||'없음'}, 자궁근종: ${p.fibroids?'있음':'없음'}, 자궁내막증: ${p.endometriosis?'있음':'없음'}, 기타: ${p.otherConditions||'없음'}, 임신 시도 기간: ${p.tryingMonths||'미입력'}개월. 이 정보를 참고해서 더 개인화된 답변을 해주세요.`;
        systemWithProfile = SYSTEM[lang] + ' ' + profileContext;
      } catch(e) {}
    }
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: newMessages, system: systemWithProfile })
      });
  ...
  function analyzeOvulation() {
    ...
    if (validDates.length < 2) { setOvulResult({ error: labels[lang].inputMore }); return; }
    const cycles = [];
    for (let i = 1; i < validDates.length; i++) {
      const diff = Math.round((validDates[i] - validDates[i-1]) / (1000*60*60*24));
      if (diff > 15 && diff < 60) cycles.push(diff);
    }
    if (cycles.length === 0) { setOvulResult({ error: labels[lang].inputMore }); return; }
          ...
          <div style={{ minHeight: 280, maxHeight: 320, overflowY: 'auto', background: '#f9fafb', borderRadius: 12, padding: 16, marginBottom: 12, border: '1px solid #e5e7eb' }}>
            ...
            {loading && (
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <div style={{ background: 'white', padding: '8px 12px', borderRadius: 12, border: '1px solid #e5e7eb', fontSize: 14 }}>⏳
